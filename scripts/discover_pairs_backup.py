...
def discover_pairs(
    ...
    median_bar_hours: Optional[float] = None
    if symbols:
        s0 = price_df[symbols[0]].dropna()
        if len(s0.index) >= 2:
            dt = s0.index.to_series().diff().dt.total_seconds().dropna() / 3600.0
            median_bar_hours = float(dt.median()) if len(dt) else None
    ...
    rej: Dict[str, object] = {
        ...
        "half_gt_max": 0,
        "pass_all": 0,
        "hl_gt_max_samples": [],  # type: ignore[list-item]
        "hl_lt_min_samples": [],
    }
    ...
    for i, sym1 in enumerate(symbols):
        for sym2 in symbols[i + 1 :]:
            ...
            if overlap_days < min_overlap_days:
                rej["fail_overlap"] = int(rej["fail_overlap"]) + 1  # type: ignore[arg-type]
                continue
            ...
            if abs(corr) < min_correlation:
                rej["fail_corr"] = int(rej["fail_corr"]) + 1  # type: ignore[arg-type]
                continue
            ...
            if p_value > max_pvalue:
                rej["fail_pvalue"] = int(rej["fail_pvalue"]) + 1  # type: ignore[arg-type]
                continue

            rej["pass_coint"] = int(rej["pass_coint"]) + 1  # type: ignore[arg-type]
            ...
            if half_life == float("inf"):
                rej["half_inf"] = int(rej["half_inf"]) + 1  # type: ignore[arg-type]
                continue
            if half_life < min_half_life_hours:
                rej["half_lt_min"] = int(rej["half_lt_min"]) + 1  # type: ignore[arg-type]
                ss = rej["hl_lt_min_samples"]
                if isinstance(ss, list) and len(ss) < 30:
                    ss.append(float(half_life))
                continue
            if half_life > max_half_life_hours:
                rej["half_gt_max"] = int(rej["half_gt_max"]) + 1  # type: ignore[arg-type]
                ss = rej["hl_gt_max_samples"]
                if isinstance(ss, list) and len(ss) < 30:
                    ss.append(float(half_life))
                continue
            ...
            rej["pass_all"] = int(rej["pass_all"]) + 1  # type: ignore[arg-type]
            results.append(
                {
                    "symbol1": sym1,
                    "symbol2": sym2,
    ...
    logger.info(
        "Discovery funnel: checked=%s overlap_fail=%s corr_fail=%s pvalue_fail=%s "
        "coint_ok=%s hl_inf=%s hl_lt_min=%s hl_gt_max=%s passed=%s median_bar_h=%s",
        rej["checked"],
        rej["fail_overlap"],
        rej["fail_corr"],
        rej["fail_pvalue"],
