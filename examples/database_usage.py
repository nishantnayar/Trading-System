...
from src.shared.database.base import db_transaction, db_readonly_session, execute_in_transaction, execute_readonly
...
def example_query_market_data():
    ...
    def get_market_data(session, symbol, days=7):
        ...
        mock_data = [
            {'symbol': symbol, 'timestamp': datetime.now(timezone.utc), 'close': Decimal('150.25')},
            {'symbol': symbol, 'timestamp': datetime.now(timezone.utc), 'close': Decimal('149.80')},
            {'symbol': symbol, 'timestamp': datetime.now(timezone.utc), 'close': Decimal('151.10')},
        ]
...
def example_complex_transaction():
    ...
    trade_id = execute_in_transaction(execute_trade_transaction, 'ORD-12345', Decimal('150.25'))
...
def example_analytics_query():
    ...
    def get_portfolio_summary(session, account_id):
        ...
        summary = [
            {'symbol': 'AAPL', 'quantity': 100, 'avg_price': Decimal('150.00'), 'current_value': Decimal('15025.00')},
            {'symbol': 'GOOGL', 'quantity': 50, 'avg_price': Decimal('2800.00'), 'current_value': Decimal('140000.00')},
            {'symbol': 'MSFT', 'quantity': 75, 'avg_price': Decimal('300.00'), 'current_value': Decimal('22500.00')},
        ]
        
        total_value = sum(record['current_value'] for record in summary)
        print(f"Portfolio Summary:")
        for record in summary:
            print(f"  {record['symbol']}: {record['quantity']} shares @ ${record['avg_price']} = ${record['current_value']}")
