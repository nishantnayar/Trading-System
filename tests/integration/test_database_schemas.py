...
class TestSchemaCreation:
    ...
    def test_all_service_schemas_exist(self, trading_engine):
        ...
        with trading_engine.connect() as conn:
            ...
            expected_schemas = [
                'analytics', 'data_ingestion', 'execution', 
                'logging', 'notification', 'risk_management', 
                'shared', 'strategy_engine'
            ]
    ...
    def test_schema_permissions(self, trading_engine):
        ...
        with trading_engine.connect() as conn:
            ...
            for schema in ['data_ingestion', 'strategy_engine', 'execution', 
                          'risk_management', 'analytics', 'notification', 
                          'logging', 'shared']:
                # Create a test table
                conn.execute(text(f"""
...
class TestDatabaseStructure:
    ...
    def test_database_encoding(self, trading_engine):
        ...
        with trading_engine.connect() as conn:
            result = conn.execute(text("SELECT pg_encoding_to_char(encoding) FROM pg_database WHERE datname = 'trading_system'"))
